...
let rec divide (l1:int list) (l2:int list) = match l2 with
  ...
  |x when (int_of_string(intlist_to_string(l1)) < int_of_string(intlist_to_string(l2))) -> ([0],l1)
...
let list_divide_rem l1 l2 = match (remove0 l1, remove0 l2) with
  (x,y) -> if(lessthan l1 l2) then remove0 l1 else (remove0 ((second_tup (divide x y))))  
...
let rem (b_int1:bigint) (b_int2:bigint) = match (remove0_bigint b_int1, remove0_bigint b_int2) with
  ...
  |((Neg,x),(_,y)) -> remove0_bigint (NonNeg, list_divide_rem x y)  
